(()=>{
 ...
 function moveProduct(order,item,dir){
  ...
  if(already){
   item.flowGroup=already.id;item.status=targetStage;
  }else if(itemsOf(order,current.id).length===1){
   current.stage=targetStage;item.status=targetStage;
  }else{
   const id=newGroupId(order);order.flowGroups.push({id,stage:targetStage});item.flowGroup=id;item.status=targetStage;
  }
 ...
 function cardHtml(order,group){
  ...
  return `<article class="compact-order-card flow-order-card" draggable="true" data-order="${order.id}" data-group="${group.id}"><div class="compact-order-heading"><div><b>${order.id}</b><small>${order.client}</small></div><span>${group.stage}</span></div><div class="compact-product-list">${productsSummary(items)}</div><div class="card-flow-actions"><button type="button" data-card-move="back" ${at<=0?'disabled':''}>←</button><small>${items.length} produto${items.length>1?'s':''}</small><button type="button" data-card-move="next" ${at>=stages.length-1?'disabled':''}>→</button></div></article>`;
 }
